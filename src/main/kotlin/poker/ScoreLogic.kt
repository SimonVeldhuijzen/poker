...
object ScoreLogic {
    fun applyShowDown(board: Board) {
        val cCards = board.communityCards
        val pCards = board.playerCards
        val bets = board.bets

        val hands = pCards.map { it + cCards }.map { cs ->
            val flush = CardSuit.values().map { s -> cs.filter { c -> c.suit == s } }.find { it.size >= 5 }
            ...
            if (isFlush) {
                ...
                if (straightFlushRank != -1) return@map StraightFlush(straightFlushRank)
            }
            val cardsByOccurrences = cs.groupingBy { it.rank.rank }.eachCount().toList().sortedByDescending { it.second * 100 + it.first }
            val ranks = cardsByOccurrences.map { it.first }.toIntArray()
            val occurences = cardsByOccurrences.map { it.second }.toIntArray()
            if (occurences[0] == 4) return@map FourOfAKind(ranks[0], ranks.copyOfRange(1, ranks.size).max()!!)
            if (occurences[0] == 3 && ranks[1] >= 2) return@map FullHouse(*ranks)
            if (isFlush) return@map Flush(*flush!!.map { it.rank.rank }.sortedDescending().toIntArray())
            val straightStrength = straightRanker(cs)
            val isStraigth = straightStrength != -1
            if (isStraigth) return@map Straight(straightStrength)
            if (occurences[0] == 3) return@map ThreeOfAKind(ranks[0], *ranks.copyOfRange(1, ranks.size))
            if (occurences[0] == 2 && occurences[1] == 2) return@map TwoPair(ranks[0], ranks[1], *ranks.copyOfRange(2, ranks.size))
            if (occurences[0] == 2) return@map Pair(ranks[0], *ranks.copyOfRange(1, ranks.size))
            return@map HighCard(*ranks)
        }
    }
}
